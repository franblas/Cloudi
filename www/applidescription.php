...
//rate et pseudo
$reponse = $bdd->prepare("SELECT note FROM rateApplication2 WHERE nomcommande=?");
$reponse->execute(array($_SESSION['descappli']));
//Affichage des messages 
while($donnees = $reponse->fetch())
{
...
<?php
$dejanote=0;
$repp1 = $bdd->prepare("SELECT ID FROM avisApplication WHERE utilisateur=? AND nomcommande=?");
$repp1->execute(array($_SESSION['login'],$_SESSION['descappli']));
$coo1=$repp1->rowCount();
if($coo1 == 1){$dejanote=1;}//si deja note
?>

<?php
//Recuperation des donnees appli
$reponse2 = $bdd->prepare("SELECT * FROM tableApplication WHERE nomcommande=?");
$reponse2->execute(array($_SESSION['descappli']));
//Affichage des messages 
...
<fieldset>
<legend id="legendvirtu"><?php echo $donnees['nomappli']; ?></legend>
<img src='/images/<?php echo $donnees['nomcommande']; ?>.png' width=180 height=180 style='float:left;margin: 0 10px 10px 0;' />
<div style='border-bottom:1px solid #000;'>
<span style='font-size: 300%;'><?php echo $donnees['nomappli']; ?></span><br/>
<?php echo $donnees['developer']; ?> , Version <?php echo $donnees['version']; ?><br/>
<?php echo $donnees['categorie']; ?><br/> 
<a href='<?php echo $donnees['site']; ?>' target=_blank ><?php echo $donnees['site']; ?></a><br/>
<form action="ciblevirtu.php" method="post"><input type="hidden" value="<?php echo $donnees['nomcommande']; ?>" name="appli" class="text"><input type="submit" class='mybutton' style='margin-top: 10px;' value="Ajouter  <?php echo $donnees['nomappli']; ?>" class="text"></form>
<form method='post' style='display:inline;' action="javascript:void(0)" onclick="document.getElementById('<?php echo 'light';echo $_SESSION['login'];echo $_SESSION['descappli'];?>').style.display='block';document.getElementById('<?php echo 'fade';echo $_SESSION['login'];echo $_SESSION['descappli']?>').style.display='block'">
...
<form action="cibleavisappli" method="post">
<input type="hidden" name="nomappli" value="<?php echo $donnees['nomcommande']; ?>" class="text">
<textarea type="text" name="avisappli" rows="5" cols="40" placeholder="Votre avis" required ></textarea><br /> 
...
$reponsee = $bdd->prepare("SELECT * FROM avisApplication WHERE nomcommande=?");
$reponsee->execute(array($donnees['nomcommande']));
//Affichage des avis
while($donneese = $reponsee->fetch())
{
if($donneese['utilisateur']!==$_SESSION['login']){//affichage avis des autres
?>
<span style='font-size:22px;'><?php echo $donneese['utilisateur'];?></span> <br/>
<span><?php echo $donneese['avis'];?> </span><br/><br/>
<?php
}
?>
...
$reponsee->execute(array($_SESSION['login'],$_SESSION['descappli']));
$cot=$reponsee->rowCount();
if($cot != 1){
?>

<!-- Fenetre popup rate -->
<div id='<?php echo 'light';echo $_SESSION['login'];echo $_SESSION['descappli'];?>' class='white_content'>
<div id='<?php echo 'rate';echo $_SESSION['login'];echo $_SESSION['descappli'];?>'>
...
<input type="hidden" value="<?php echo $_SESSION['descappli'];?>" name="appapp" class="text">
...
<a class='closepopup' href='javascript:void(0)' onclick="document.getElementById('<?php echo 'light';echo $_SESSION['login'];echo $_SESSION['descappli'];?>').style.display='none';document.getElementById('<?php echo 'fade';echo $_SESSION['login'];echo $_SESSION['descappli'];?>').style.display='none' "><img src='/images/crossclose.png' width=90% /></a>
</div>
<div id='<?php echo 'fade';echo $_SESSION['login'];echo $_SESSION['descappli'];?>' class='black_overlay' onclick="document.getElementById('<?php echo 'light';echo $_SESSION['login'];echo $_SESSION['descappli'];?>').style.display='none';document.getElementById('<?php echo 'fade';echo $_SESSION['login'];echo $_SESSION['descappli'];?>').style.display='none' "></div>
<?php
}//if end non note
?>
